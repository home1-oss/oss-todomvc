...
const PATHS = {
    app: path.join(__dirname, './src/main/index.js'),
    devBuild: __dirname,
    ...
    module: {
        loaders: [
            ...
            { test: /\.svg(\?v=\d+\.\d+\.\d+)?$/, loader: 'file?name=images/[name].[hash].[ext]&mimetype=image/svg+xml' },
            {
                ...
                test: /\.jsx?$/,
                loader: "babel-loader",
                // Skip any files outside of your project's `src/main` directory
                include: [
...
const dev = {
    ...
    entry: {
        app: [
            'webpack-dev-server/client?http://127.0.0.1:3000', // Enables websocket connections (needs url and port)
